...
class PatchedLlamaAttention(LlamaAttention):
    """
    Copy of HF LlamaAttention with a two‑line AdaptVis scale:
    we multiply the raw logits of the *last query row*
    that attends to the image‑patch keys by a scalar weight.
    """

    def __init__(self, orig: LlamaAttention):
        super().__init__(orig.config)                 # build an empty shell
        self.load_state_dict(orig.state_dict())       # copy all parameters
    ...
    def forward(
        ...
        *,
        ...
        keys_mask: Optional[torch.Tensor] = None,     # NEW  (1×seq bool)
        ...
        if past_key_value is not None:                # KV‑cache merge
            key   = torch.cat([past_key_value[0], key], dim=1)
            value = torch.cat([past_key_value[1], value], dim=1)

        # rotary positional embedding (same as HF impl.)
        # ---  omitted for brevity  ---

        attn_logits = (query @ key.transpose(-2, -1)) * (self.head_dim ** -0.5)

        # ---------- AdaptVis scaling (single coefficient α) ----------
        if weight != 1.0 and keys_mask is not None:
            img_idx = keys_mask.nonzero(as_tuple=True)[1]  # (N_img,)
            attn_logits[:, :, -1, img_idx] *= weight       # last query row

        # ---------- soft‑max & value projection (unchanged) ----------
        if attention_mask is not None:
            attn_logits = attn_logits + attention_mask
        attn_probs  = nn.functional.softmax(attn_logits, dim=-1, dtype=query.dtype)
...
def inject_adaptvis_attention(model):
    """
    Walk the module tree once and replace every LlamaAttention
    with the patched version defined above.
    """
    for name, module in list(model.named_modules()):
        if isinstance(module, LlamaAttention):
            parent_name, child_name = name.rsplit(".", 1)
            parent = model.get_submodule(parent_name)
            setattr(parent, child_name, PatchedLlamaAttention(module))

def build_keys_mask(input_ids, img_token_id: int = -200, n_patches: int = 256):
    """
    Returns a 1×seq boolean mask that is True for the image token itself
    and the following n_patches patch tokens.
    """
    mask = torch.zeros_like(input_ids, dtype=torch.bool)
    pos = (input_ids == img_token_id).nonzero(as_tuple=True)
    if len(pos[0]) > 0:
        start = pos[1][0].item()
        mask[:, start : start + n_patches + 1] = True
    return mask
...
class DecoderArguments:
    ...
    confidence_threshold_low: float = field(default=0.7)
...
def ratio_sequence(input_ids, attention, num_of_images, target_number):
    layer_number = 16
...
def ratio_collage( input_ids, attention, image_dims, block_index):
...
def ratio_pip(input_ids, attention):
    layer_number = 16
    ### 找到所有 <image> 的起始位置
    indices = torch.nonzero(input_ids[0] == -200, as_tuple=False).squeeze().tolist()
...
def calculation_attention_ratio( input_ids, attention, num_of_images, target_number, image_type, image_dims):
    input_ids = input_ids.unsqueeze(0)

    if image_type == "sequence":
        return ratio_sequence(input_ids, attention, num_of_images, target_number)

    elif image_type == "collage":
        return ratio_collage(input_ids, attention, image_dims, target_number)
    elif image_type == "pip":
        return ratio_pip(input_ids, attention)
    else:
...
def evaluate(attn_implementation):
    ...
    inject_adaptvis_attention(model)
    print( model )

    gen_conf = model.generation_config
    extra_names = set(getattr(gen_conf, "model_kwarg_names", []))
    extra_names.update(["weight", "keys_mask"])
    gen_conf.model_kwarg_names = list(extra_names)
    ...
    for batch in tqdm( eval_dataloader, desc="Evaluating"):
        ...
        with torch.inference_mode():
            ...
            first_conf = torch.softmax(first_logits, -1).max().item()
        
        if first_conf < decoder_args.confidence_threshold_low:
            alpha = decoder_args.alpha_low     # e.g. 0.5  (smooth)
        elif first_conf > decoder_args.confidence_threshold_high:
            alpha = decoder_args.alpha_high    # e.g. 2.0  (sharpen)
        else:
            alpha = 1.0
            
        keys_mask = build_keys_mask(prompt_ids)
        output_ids = model.generate(
            prompt_ids,
            images=[image.to(dtype=torch.bfloat16 if model_args.bf16 else torch.float16).unsqueeze(0), keys], 
            weight=alpha,                  # propagated to every layer
            keys_mask=keys_mask,
        ...
        attention_ratios = calculation_attention_ratio( prompt_ids.squeeze(), attention, num_of_images, target_number, image_type, image_dims)
        ...
        ans_file.write(json.dumps({"question_id": idx,
                                   "text": outputs,
                                   "attention_ratio": attention_ratios
                                   }
                        ) + "\n")
...
if __name__ == "__main__":
    evaluate(attn_implementation="eager")
