...
def evaluate(attn_implementation):
    ...
    for batch in tqdm( eval_dataloader, desc="Evaluating"):
        ...
        with torch.inference_mode():
            ...
            def expand_tensor_on_trigger(input_tensor, trigger=-200, insert_len=576):
                output = []
                for val in input_tensor:
                    if val == trigger:
                        output.extend([1] * insert_len)  # insert 576 ones
                    else:
                        output.append(0)
                return torch.tensor(output, dtype=torch.int)

            image_indices = expand_tensor_on_trigger(prompt_ids[0])
            ...
            print(uncertainty, decoder_args.confidence_threshold_low)

            # Adjust attention based on uncertainty
            if uncertainty < decoder_args.confidence_threshold_low:
                output = model.generate(
                    prompt_ids,
                    images=[image.to(dtype=torch.bfloat16 if model_args.bf16 else torch.float16).unsqueeze(0), keys],
                    do_sample=True if decoder_args.temperature > 0 else False,
                ...
                output = model.generate(
                    ...
                    use_cache=True,
                    output_attentions=True,
                    weight=decoder_args.alpha_high,
                    keys=image_indices,
                    output_scores=True, return_dict_in_generate=True
                )
        ...
        ans_file.write(json.dumps({"question_id": idx,
                                   "text": outputs,
                                   "attention_ratio": str(attention_ratios)
                                   }
                        ) + "\n")
        # ans_file.flush()
    ans_file.close()
