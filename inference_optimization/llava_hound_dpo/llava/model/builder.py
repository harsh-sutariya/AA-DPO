...
def load_pretrained_model(model_path, model_base, model_name, image_tower_model_args, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, legacy=False, bf16 = True, image_tower_path = "openai/clip-vit-large-patch14-336", base_model_only = False, adaptive=True):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None and base_model_only == False:
            lora_cfg_pretrained = AutoConfig.from_pretrained(model_path)
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            print(f'Loading LLaVA from base model:\n{model_base}')
            model = LlavaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
            # from .llava15 import LlavaWrapper
            # model = LlavaWrapper(root_dir=CACHE_DIR, device=device, method="adapt_vis")
            ...
            if base_model_only:
                print("Only loading the base_model with model_path configs")
        ...
        if 'Image' in X:
            ...
            if image_tower is None:
                model.get_model().initialize_image_modules(
                    model_args=image_tower_model_args
                )
