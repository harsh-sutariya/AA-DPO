...
class DPOTrainer(Trainer):
    ...
    def ratio_sequence(self, input_ids, attention, num_of_images, target_number, layer_number):
        ...
        output_token_attention = [
            attention[i + 1][layer_number][0] for i in range(output_token_length)
        ]

        max_tensor_size = len(output_token_attention) + len(attention[0][layer_number][0])
        merged_tensor = torch.zeros(
            len(attention[0][layer_number][0]), max_tensor_size, max_tensor_size,
            device=input_attention.device, dtype=input_attention.dtype
        )
        ...
        image_tensors = [
            merged_tensor[-output_token_length:, indices[i]:indices[i] + 576]
        ...
        means = torch.stack([torch.mean(tensor) for tensor in reshape_image_tensors])
        ratio = means[target_number] / torch.sum(means)

        return ratio

    def ratio_pip(self, input_ids, attention, layer_number):
        ...
        input_attention = torch.mean(attention[0][layer_number][0], dim=0)
        output_token_length = len(attention) - 1
        output_token_attention = [attention[i + 1][layer_number][0] for i in range(output_token_length)]

        max_tensor_size = len(output_token_attention) + len(attention[0][layer_number][0])
        merged_tensor = torch.zeros(len(attention[0][layer_number][0]), max_tensor_size, max_tensor_size, device=input_attention.device)
        ...
        # Get attention map for the first image
        image_tensors = [merged_tensor[-(len(output_token_attention)):, indices[0]:indices[0] + 576]]
        mean_image_tensors = [torch.mean(image_tensor, dim=0) for image_tensor in image_tensors]
        reshape_image_tensors = [mean_image_tensor.reshape(24, 24) for mean_image_tensor in mean_image_tensors]

        final_tensor = torch.cat(reshape_image_tensors, dim=1)

        if final_tensor.shape != (24, 24):
            raise ValueError("Input tensor must be of shape 24x24")

        # Get 16x16 center crop
        start = (24 - 16) // 2
        end = start + 16
        center_region = final_tensor[start:end, start:end]

        center_sum = torch.sum(center_region)
        total_sum = torch.sum(final_tensor)
    ...
    def ratio_collage(self, input_ids, attention, image_dims, block_index, layer_number):
        ...
        height_cumsum = torch.cumsum(torch.tensor([0] + block_heights, device=final_tensor.device), dim=0)
        width_cumsum = torch.cumsum(torch.tensor([0] + block_widths, device=final_tensor.device), dim=0)

        row_idx = (block_index - 1) // cols
        col_idx = (block_index - 1) % cols

        h_start = height_cumsum[row_idx]
        h_end = height_cumsum[row_idx + 1]
        w_start = width_cumsum[col_idx]
        w_end = width_cumsum[col_idx + 1]

        block = final_tensor[h_start:h_end, w_start:w_end]
    ...
    def calculation_attention_ratio(self, input_ids, attention, num_of_images, target_number, image_type, image_dims, layer_idx):
        input_ids = input_ids.unsqueeze(0)

        if image_type == "sequence":
            return self.ratio_sequence(input_ids, attention, num_of_images, target_number, layer_idx)

        elif image_type == "collage":
            return self.ratio_collage(input_ids, attention, image_dims, target_number, layer_idx)
        elif image_type == "pip":
            return self.ratio_pip(input_ids, attention, layer_idx)
        else:
    ...
    def attention_loss(self, attention_ratios, threshold=0.75):
        """
        Compute the attention loss as max(0, threshold - attention_ratio) (hinge-style loss),
        in a differentiable way.

        Args:
            attention_ratios: List of attention ratios (torch tensors with grad)
            threshold: Minimum acceptable attention ratio

        Returns:
            torch.Tensor containing the mean attention loss
        """
        # Convert threshold to tensor on the correct device
        threshold_tensor = torch.tensor(threshold, device=attention_ratios[0].device, dtype=attention_ratios[0].dtype)
    ...
    def get_batch_loss_metrics(
        ...
        for batch_idx in range(batch['chosen_input_ids'].shape[0]):
            ...
            for i in range(14, 23):
                local_ += self.calculation_attention_ratio(
            ...
            local_ = local_ / (22 - 14 + 1 )
            attention_ratios.append(local_)
    ...
    def compute_loss(
        ...
        def check_grad_hook(name):
            def hook(grad):
                print(f"[GRADIENT] {name}: {grad.abs().mean().item():.6f}")
            return hook


        layers = model.module.base_model.model.model.layers
        for i in range(14, 23):
            layer = layers[i]
            for proj_name in ["q_proj", "k_proj"]:
                proj = getattr(layer.self_attn, proj_name, None)
                if proj and hasattr(proj, "lora_A"):
                    print("lora_A: ", proj.lora_A)
                    lora_A = proj.lora_A["default"]
                    try:
                        lora_A.weight.register_hook(check_grad_hook(f"layer{i}.{proj_name}.lora_A"))
                    except Exception as e:
                        print(f"Failed to register hook on layer {i} {proj_name} lora_A: {e}")
